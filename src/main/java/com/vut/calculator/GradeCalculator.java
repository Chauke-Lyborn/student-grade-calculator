 ...
 *
 * ===== GRADING RULES (as per VUT policy) ===== Final Mark = (Semester Mark *
 * 0.4) + (Exam Mark * 0.6)
 *
 * Grade Boundaries: 80-100 -> Distinction 70-79 -> Merit 60-69 -> Credit 50-59
 * -> Pass 0-49 -> Fail
 *
...
public class GradeCalculator {
    ...
    /**
     * BUG #1: Weight calculation is swapped Semester mark should be 40% and
     * exam should be 60%, but the weights are reversed here.
     */
    ...
    /**
     * BUG #2: Grade boundaries are incorrect - Distinction should be >= 80, but
     * uses > 80 (misses exactly 80) - Pass boundary uses >= 55 instead of >= 50
    ...
    /**
     * BUG #3: Admission check uses wrong threshold Should require >= 40 for
     * exam admission, but uses >= 45
     */
    ...
    /**
     * BUG #4: Average calculation has off-by-one error Divides by (marks.length
     * + 1) instead of marks.length
     */
    ...
    /**
     * BUG #5: Pass rate calculation divides by total instead of multiplying by
     * 100 Also uses wrong threshold (>= 55 instead of >= 50)
     */
    ...
    /**
     * BUG #6: Highest mark finder returns lowest instead Uses < comparison instead of
     * >
     */
    ...
    /**
     * BUG #7: Supplementary exam eligibility check is wrong A student qualifies
     * for a supplementary if their final mark is between 45 and 49 (inclusive),
     * but this method checks 40-44
     */
    ...
    /**
     * BUG #8: Mark validation allows marks over 100 and below 0 Should return
     * false for marks < 0 or > 100 Currently returns true for any value
     */
    ...
    /**
     * Returns a formatted student report string Contains BUG #9: Report shows
     * "ADMITTED" even when student is NOT admitted (the condition is inverted)
     */
